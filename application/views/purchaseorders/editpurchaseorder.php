...
<form method="post">
	...
	<div class="row">
		<?php if ($purchaseOrder->SupplierId == 0) { ?>
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						...
						<div class="row">
							<div class="col-12 form-group label-floating">
								...
								<input type="text" name="company_name" class="form-control" value="<?= $purchaseOrder->CompanyName ?>" />
							</div>
							<div class="col-md-4 form-group label-floating">
								<label class="control-label">Voornaam</label>
								<input type="text" name="front_name" class="form-control" value="<?= $purchaseOrder->FrontName ?>" />
							</div>
							<div class="col-md-4 form-group label-floating">
								<label class="control-label">Tussenvoegsel</label>
								<input type="text" name="insertion" class="form-control" value="<?= $purchaseOrder->Insertion ?>" />
							</div>
							<div class="col-md-4 form-group label-floating">
								<label class="control-label">Achternaam</label>
								<input type="text" name="last_name" class="form-control" value="<?= $purchaseOrder->LastName ?>" />
							</div>
						</div>
						<div class="row">
							<div class="col-md-4 form-group label-floating">
							...
							<div class="col-md-2 form-group label-floating">
								...
								<input type="number" name="address_number" class="form-control" value="<?= $purchaseOrder->AddressNumber ?>" />
							</div>
							<div class="col-md-2 form-group label-floating">
								<label class="control-label">Toevoeging</label>
								<input type="text" name="address_addition" class="form-control" value="<?= $purchaseOrder->AddressAddition ?>" />
							</div>
						</div>
						<div class="row">
							<div class="col-md-3 form-group label-floating">
								<label class="control-label">Postcode</label>
								<input type="text" name="zip_code" class="form-control" value="<?= $purchaseOrder->ZipCode ?>" />
							</div>
							<div class="col-md-5 form-group label-floating">
								<label class="control-label">Woonplaats</label>
								<input type="text" name="city" class="form-control" value="<?= $purchaseOrder->City ?>" />
							</div>
							<div class="col-md-4 form-group label-floating">
								<label class="control-label">Land</label>
								<input type="text" name="country" class="form-control" value="<?= $purchaseOrder->Country ?>" />
							</div>
						</div>
						<div class="row">
							<div class="col-12 form-group label-floating">
								<label class="control-label">E-mailadres</label>
								<input type="text" name="mail_address" class="form-control" value="<?= $purchaseOrder->MailAddress ?>" />
							</div>
						</div>
					</div>
				</div>
					...
					<div class="row mt-2">
						<div class="col-md-3 input-group">
							...
							<div class="form-group label-floating">
								...
								<input class="form-control date-picker" type="text" name="orderdate" value="<?= date('d-m-Y', strtotime($purchaseOrder->OrderDate)); ?>" required />
							</div>
						</div>

						<div class="col-md-3 form-group label-floating">
							<label class="control-label">Referentie</label>
							<input name="reference" value="<?= $purchaseOrder->Reference; ?>" class="form-control" type="text" autocomplete="off" />
						</div>
					</div>
				</div>
			</div>
							...
							<table class="table table-borderless" id="tableInvoice">
								...
								<tbody>
									...
									foreach ($purchaseOrderRules as $purchaseOrderRule) {
										...
										<tr id="orderRow<?= $i ?>">
											<td>
												<input name="articlenumber<?= $i; ?>" id="articlenumber" class="form-control" type="text" autocomplete="off" onkeyup="searchArticlenumber(this, <?= $i ?>)" value="<?= $purchaseOrderRule->ArticleC; ?>" required />
												<ul class="list-group suggestions-listgroup clickable"></ul>
											</td>
											<td>
												<input name="ean_code<?= $i ?>" id="ean_code" class="form-control" type="text" autocomplete="off" onkeyup="searchEanCode(this, <?= $i ?>)" value="<?= $purchaseOrderRule->EanCode; ?>" />
												<ul class="list-group suggestions-listgroup clickable"></ul>
											</td>
											<td>
												<input name="articledescription<?= $i; ?>" class="form-control" type="text" autocomplete="off" onkeyup="searchDescription(this, <?= $i ?>)" value="<?= $purchaseOrderRule->Description; ?>" />
												<ul class="list-group suggestions-listgroup clickable"></ul>
											</td>
											<td><input name="amount<?= $i; ?>" class="form-control" type="number" value="<?= $purchaseOrderRule->Amount; ?>" onchange="calculateRow(<?= $i; ?>)" step="any"  /></td>
											<td><input name="purchaseprice<?= $i; ?>" class="form-control" type="number" value="<?= $purchaseOrderRule->Price; ?>" onchange="calculateRow(<?= $i; ?>)" step="any" /></td>
											<td><input name="discount<?= $i; ?>" class="form-control" type="number" value="<?= $purchaseOrderRule->Discount; ?>" onchange="calculateRow(<?= $i; ?>)" /></td>
											<td>
												<div class="form-group">
													<select name="tax<?= $i; ?>" class="form-control" onchange="calculateRow(<?= $i; ?>)">
														<option <?= $purchaseOrderRule->Tax == 21 ? 'selected' : ''; ?> value="21">21%</option>
														<option <?= $purchaseOrderRule->Tax == 9 ? 'selected' : ''; ?> value="9">9%</option>
														<option <?= $purchaseOrderRule->Tax == 0 ? 'selected' : ''; ?> value="0">0%</option>
													</select>
												</div>
											</td>
											<td><input name="total<?= $i; ?>" class="form-control" type="number" value="<?= $purchaseOrderRule->Total; ?>" readonly /> <input id="number" name="number[]" type="hidden" readonly value="<?= $i; ?>" /></td>
											<td class="td-actions text-right">
												<button type="button" onclick="removeRow(<?= $i ?>)" class="btn btn-danger btn-round btn-fab btn-fab-mini">
													<i class="material-icons">close</i>
												</button>
												<input type="hidden" name="ruleNumber<?= $i; ?>" value="<?= $purchaseOrderRule->Id; ?>" />
											</td>
										</tr>
										<?php
										$i++;
