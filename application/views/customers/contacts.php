<?php
defined('BASEPATH') OR exit('No direct script access allowed');

define('TITEL', 'Contactpersonen');
define('CUSTOMER', $this->uri->segment(3));
define('SUBTITEL', 'Contactpersonen: ' . getCustomerName($this->uri->segment(3)) . ' (' . $this->uri->segment(3) . ')');
define('PAGE', 'customer');
define('SUBMENUPAGE', 'contacts');
define('SUBMENU', $this->load->view('customers/tab', array(), true));

include 'application/views/inc/header.php';
?>
<div class="row">
	<?= SUBMENU; ?>
	<div class="col-auto">
		<a class="btn btn-success" href="<?= base_url('customers/createcontact/'.$this->uri->segment(3)) ?>">Contactpersoon toevoegen</a>
	</div>
</div>

<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
		<div class="card">
			<div class="card-header card-header-icon card-header-primary">
				<div class="card-icon">
					<i class="material-icons">local_offer</i>
				</div>
				<h4 class="card-title">Contactpersonen</h4>
			</div>
			<div class="card-body">
				<div class="table-responsive">
					<table class="table table-striped table-hover" id="contactsTable" width="100%">
						<thead>
							<tr>
								<td>Voornaam</td>
								<td>&nbsp;</td>
								<td>Achternaam</td>
								<td>E-mailadres</td>
								<td>Telefoon</td>
							</tr>
						</thead>
						<tbody>
							<?php foreach ($contacts as $contact) { ?>
								<tr data-href="<?= base_url('customers/updatecontact/'.$contact->Id) ?>">
									<td><?= $contact->FirstName; ?></td>
									<td><?= $contact->Insertion; ?></td>
									<td><?= $contact->LastName; ?></td>
									<td><?= $contact->Email; ?></td>
									<td><?= !empty($contact->PhoneNumber) ? $contact->PhoneNumber : $contact->PhoneMobile; ?></td>
								</tr>
							<?php } ?>
						</tbody>
					</table>
				</div>
			</div>
		</div>
	</div>
</div>

<script>

$(document).ready(function () {
	
	$('#contactsTable').DataTable({
		"language": {
			"url": "/assets/language/Dutch.json"
			
		},
		"order": [[2, "asc"]]
	});
	
});

</script>

<?php include 'application/views/inc/footer.php'; ?>
