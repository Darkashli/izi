...
define('TITEL', 'Verkooporders');
define('PAGE', 'salesorder');
...
<form method="post">
	<div class="row">
		<?php if ($salesOrder->CustomerId == 0) { ?>
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						...
						<div class="row">
							...
							<div class="col-md-6 col-sm-12 form-group label-floating">
								...
								<input name="termofpayment" class="form-control" type="number" value="<?= $salesOrder->TermOfPayment ?>" required/>
							</div>
						</div>
					</div>
				</div>
								...
								<tbody>
									...
									foreach ($salesOrderRules as $salesOrderRule) {
										...
										<tr id="orderRow<?= $i; ?>">
											...
											<td class="align-top"><input name="amount<?= $i; ?>" class="form-control" type="number" value="<?= $salesOrderRule->Amount; ?>" onchange="calculateRow(<?= $i; ?>)" step="any"  /></td>
											<td class="align-top"><input name="salesprice<?= $i; ?>" class="form-control" type="number" value="<?= $salesOrderRule->Price; ?>" onchange="calculateRow(<?= $i; ?>)" step="any" /></td>
											<td class="align-top"><input name="discount<?= $i; ?>" class="form-control" type="number" value="<?= $salesOrderRule->Discount; ?>" onchange="calculateRow(<?= $i; ?>)" /></td>
											<td class="align-top">
												<div class="form-group">
													<select name="tax<?= $i; ?>" class="form-control" onchange="calculateRow(<?= $i; ?>)">
														<option <?= $salesOrderRule->Tax == 21 ? 'selected' : ''; ?> value="21">21%</option>
														<option <?= $salesOrderRule->Tax == 9 ? 'selected' : ''; ?> value="9">9%</option>
														<option <?= $salesOrderRule->Tax == 0 ? 'selected' : ''; ?> value="0">0%</option>
													</select>
												</div>
											</td>
											<td class="align-top"><input name="total<?= $i; ?>" class="form-control" type="number" value="<?= $salesOrderRule->Total; ?>" readonly /> <input id="number" name="number[]" type="hidden" readonly value="<?= $i; ?>" /></td>
											<td class="align-middle td-actions text-right">
												...
												<input type="hidden" name="ruleNumber<?= $i; ?>" value="<?= $salesOrderRule->Id; ?>" />
											</td>
										</tr>
										<?php
										$i++;
		...
		<div class="col-12">
			<div class="card">
				...
				<div class="card-body">
					<div class="row mt-2">
						<div class="col-12">
							<textarea name="note" class="editortools" rows="10"><?= $salesOrder->Note ?></textarea>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	var currentRow = <?= $i ?>;
	var articles = {};
	var customerId = <?= $salesOrder->CustomerId != 0 ? $customer->Id : 'null' ?>;
